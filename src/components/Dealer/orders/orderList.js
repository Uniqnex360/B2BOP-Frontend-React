...
const OrderList = () => {
  ...
  const [page, setPage] = useState(initialPage);
    ...
    useEffect(() => {
      if(searchQuery){
      setPage(0); 
      }// Reset page to 0 when searchQuery is updated
    }, [searchQuery]); // This effect runs whenever searchTerm changes
...
const handleOrderClick = (orderId) => {
  ...
  // Filter the orders based on current page and rows per page
  ...
  return (
    ...
    paginatedOrders.map((order) => (
        ...
        <TablePagination
          ...
          component="div"
          count={filteredOrders.length} // Total number of items
